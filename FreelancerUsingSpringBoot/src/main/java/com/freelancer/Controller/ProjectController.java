...
public class ProjectController {
            ...
            if (p != null) {
                ...
                for (Bid bid:bids) {
                    ...
                    if(bid.getUserId()==freelancer_id){
                        ...
                        p.setProjectStatus("NOT_DELIVERED");
                        p.setPaymentStatus("PAYMENT_PENDING");
                    }else{
    ...
    @PostMapping(path = "/markProjectAsCompleted", consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> markProjectAsCompleted(@RequestBody String projectDetails, HttpSession session) {
        ResultObject ro = new ResultObject("Error marking project as completed", "", null);
        ...
        try {
            ...
            if (p != null) {
                ...
                ro.setErrorMsg("");
                ro.setSuccessMsg("Project marked as completed successfully");
                ro.setData(p);
                logger.info("Project ID " + projectId + " marked as completed");
                return new ResponseEntity<>(ro, HttpStatus.OK);
            } else {
                ro.setErrorMsg("Project not found");
                ro.setSuccessMsg("");
            ...
            logger.error(e.getMessage());
            ro.setErrorMsg("Error marking project as completed");
            ro.setSuccessMsg("");
            return new ResponseEntity<>(ro, HttpStatus.BAD_REQUEST);
        }
    }
    ...
    public ResponseEntity<?> getUserBidProjects(@RequestBody String userDetails, HttpSession session) {
        ...
        try{
            JSONObject jsonObject = new JSONObject(userDetails);
            if(jsonObject.has("id")){
                List bidData =  objectService.getUserBidProjects(Long.parseLong(jsonObject.getString("id")));
                List hireData =  objectService.getUserHiredProjects(Long.parseLong(jsonObject.getString("id")));
                Object  data[] = {bidData, hireData};
                ro.setErrorMsg("");
                ro.setSuccessMsg("User Projects Fetched");
                ro.setData(data);
                logger.info("User Projects Fetched");
