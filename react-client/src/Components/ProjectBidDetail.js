...
class ProjectBidDetail extends Component {
  ...
  renderImage(id, profile_id) {
    if (!profile_id) {
      return <img className="card-img-right flex-auto d-none d-md-block" alt="Thumbnail [200x250]"
  ...
  render() {
    ...
    let projectBids = this.state.projectBids.map(bid => {
      return (
        <div>
          <div className="card flex-md-row mb-8 box-shadow h-md-300">
            <div className="card-body d-flex flex-column align-items-start ">
              <h3> <strong className="d-inline-block mb-1 text-primary">{bid.userName}</strong></h3>

              <div className="mb-1"><strong><p className="card-text mb-auto text-muted">Days : {bid.bid_period}</p></strong></div>
              <strong><p className="card-text mb-auto text-muted">Bid : ₹ {bid.bid_amount}</p></strong>
              {localStorage.getItem('hireFlag') == 'true' ?
                <button type="button" id={bid.userId + '/' + bid.bid_period} onClick={this.hireEmployer} className="btn btn-primary">Hire {bid.name}</button>
                : <div />}
            </div>
